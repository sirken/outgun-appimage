...
// Best-effort recursive mkdir, e.g. for a whereuserdir under $HOME that may not exist at all yet ("mkdir -p").
// Errors (including "already exists") on intermediate components are ignored; that's the expected common case.
static void makeDirRecursive(const string& path) throw () {
    for (size_t pos = 1; pos < path.length(); ++pos) {
        if (path[pos] == '/')
            platMkdir(path.substr(0, pos));
    }
...
void platInitAfterAllegro() throw () {
    ...
    // initialize whereuserdir: writable per-user data root, separate from the (possibly read-only, e.g. AppImage/Flatpak) install location in wheregamedir
    const char* xdgDataHome = getenv("XDG_DATA_HOME");
    if (xdgDataHome && *xdgDataHome)
        whereuserdir = string(xdgDataHome) + "/outgun/";
    else {
        const char* home = getenv("HOME");
        whereuserdir = (home && *home) ? string(home) + "/.local/share/outgun/" : wheregamedir;
    }
    if (whereuserdir != wheregamedir)
        makeDirRecursive(whereuserdir);
