...
Server::Server(LogSet& hostLogs, const ServerExternalSettings& config, Log& externalErrorLog, const string& errorPrefix) throw () :
    normalLog(whereuserdir + "log" + directory_separator + "serverlog.txt", true),
    errorLog(normalLog, externalErrorLog, "ERROR: ", errorPrefix),
    securityLog(normalLog, "SECURITY WARNING: ", whereuserdir + "log" + directory_separator + "server_securitylog.txt", false),
    adminActionLog(normalLog, "ADMIN ACTION: ", whereuserdir + "log" + directory_separator + "adminactionlog.txt", false),
    log(&normalLog, &errorLog, &securityLog),
    threadLock(config.threadLock),
    threadLockMutex("Server::threadLockMutex"),
    abortFlag(false),
    quit_bots(false),
    botTestMode(config.botTestMode),
    botReactedFrame(0),
...
bool Server::load_rotation_map(int pos) throw () {
    ...
    if (maprot[pos].random) {
        ...
        world.generate_map(dir, file_name, maprot[pos]);
    }
    else {
        dir = SERVER_MAPS_DIR;
...
bool Server::server_next_map(int reason, const string& currmap_title_override) throw () {
    ...
    if (botTestMode && world.frame > 100) { // there's a special restart on frame 100
        static int points[2], wins[2], draws = 0;
        for (int t = 0; t < 2; ++t) {
...
void Server::start_recording() throw () {
    ...
    if (settings.get_recording()) {
        ...
        record_filename = whereuserdir + "replay" + directory_separator + time_str + ".replay";
...
string Server::create_bot_name() throw () {
    ...
    if (!file.empty()) {
        ...
        if (file[0] != directory_separator && file.find(':') == string::npos)
            file = whereuserdir + "config" + directory_separator + file;
    ...
    if (message[0] == '/') {
        ...
        if (command == "help") {
            ...
            if (arguments.find_first_not_of(" ") != string::npos) {
                ofstream log((whereuserdir + "log" + directory_separator + "sayadmin.log").c_str(), ios::out | ios::app);
