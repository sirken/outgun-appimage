...
// Make directory under the (possibly read-only) game directory if it does not already exist.
static bool check_dir(const string& dir, LogSet& log) throw () {
    return check_absolute_dir(wheregamedir + dir, log);
}

// Make directory under the writable per-user data directory if it does not already exist.
static bool check_user_dir(const string& dir, LogSet& log) throw () {
    return check_absolute_dir(whereuserdir + dir, log);
}

// Copy a file shipped read-only under wheregamedir/config to whereuserdir/config, if it doesn't already exist there.
// Used for config files that either can't gracefully default (auth.txt) or ship valuable reference content (gamemod.txt).
static void seedUserConfigFileIfMissing(const string& name, LogSet& log) throw () {
    const string dest = whereuserdir + "config" + directory_separator + name;
    if (platIsFile(dest))
        return;
    const string src = wheregamedir + "config" + directory_separator + name;
    ifstream in(src.c_str(), std::ios::binary);
    if (!in)
        return;
    std::ofstream out(dest.c_str(), std::ios::binary);
    if (!out) {
        log.error(_("Can't write '$1'.", dest));
        return;
    }
    out << in.rdbuf();
}
...
bool load_language(LogSet& log) {
    const string lang_file = whereuserdir + "config" + directory_separator + "language.txt";
...
static int wrappedMain(int argc, const char* argv[]) throw () {
    ...
    if (!check_user_dir("log", noLogSet)) {
...
static void innerMain(int argc, const char* argv[], LogSet& log, MemoryLog& memoryErrorLog) throw () {
    ...
    (void)showFirstTimeSplash; // avoid compilation warning of unused variable for dedicated server
    {
    ...
    check_user_dir("config", log);
    seedUserConfigFileIfMissing("auth.txt", log);
    seedUserConfigFileIfMissing("gamemod.txt", log);
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-ded"))
            ...
            string target = argc >= 4 ? argv[3] : whereuserdir + "mappic";
            if (argc >= 5)
    ...
    check_dir(SERVER_MAPS_DIR, log);    // the client might run a server, so check these in any case; SERVER_MAPS_DIR itself ships bundled maps read-only
    check_user_dir(SERVER_MAPS_DIR, log); // parent must exist before creating its "generated" subdirectory below
    check_user_dir(SERVER_MAPS_DIR + directory_separator + GENERATED_MAPS_SUBDIR_NAME, log); // server-generated maps are writable per-user data
    check_user_dir("server_stats" , log);
    check_user_dir("replay"       , log);
    ...
    // run client
        ...
        check_dir(CLIENT_MAPS_DIR, log); // CLIENT_MAPS_DIR ships bundled example maps read-only
        check_user_dir(CLIENT_MAPS_DIR, log); // but is also where downloaded maps are cached, which is writable per-user data
        check_user_dir("screens"      , log);
        check_dir("graphics"     , log);
        check_dir("sound"        , log);
        check_user_dir("client_stats" , log);
        ...
        FileLog clientLog(whereuserdir + "log" + directory_separator + "clientlog.txt", true);
        if (!language_loaded) {
