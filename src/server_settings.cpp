...
void Server::SettingManager::build(bool reload) throw () {
    ...
    cat.add(new GS_Ignore    ("tournament"));
    ...
    cat.add(new GS_Boolean   ("capture_away_from_base",      &worldConfig.capture_away_from_base));
    cat.add(new GS_Boolean   ("carry_own_team_flag",         &worldConfig.carry_own_team_flag));
...
void Server::SettingManager::loadGamemod(bool reload) throw () {
    build(reload);
    const string filename = whereuserdir + "config" + directory_separator + "gamemod.txt";
