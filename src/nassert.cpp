...
void nasprintf(const char* file, int line, const char* expr, ...) throw () {
    ...
    #ifndef DISABLE_ENHANCED_NASSERT
    // save to assert.log
    FILE* asfile = fopen((whereuserdir + "log" + directory_separator + "assert.log").c_str(), "at");
    if (asfile) {
        ...
        fprintf(asfile, "\n");
        fclose(asfile);
        FILE* stdump = fopen((whereuserdir + "log" + directory_separator + "stackdump.bin").c_str(), "wb");
        if (stdump) {
            stackDump(stdump);
            fclose(stdump);
        }
