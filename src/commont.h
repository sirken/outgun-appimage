...
extern const std::string GENERATED_MAPS_SUBDIR_NAME; // subdirectory name (no separator) of SERVER_MAPS_DIR holding server-generated maps

// system directory separator
extern char directory_separator;

// root path (game executable path); read-only bundled assets (graphics, sound, fonts, languages, mapgen, shipped maps) live here
extern std::string wheregamedir;

// root path for writable per-user data (config, logs, replays, stats, screenshots, downloaded/generated maps)
extern std::string whereuserdir;
